...
import { Calendar, MapPin, AlertTriangle, CheckCircle, Clock, Truck, Plus, Settings, Navigation, Wrench, Locate, Wifi } from 'lucide-react';
import { mockReports, mockIncidents } from '../../data/mockData';
import { IncidentModal } from '../Common/IncidentModal';
import { useAuth } from '../../context/AuthContext';
import { trucksAPI, collectionPointsAPI, incidentsAPI, schedulesAPI } from '../../services/api';
import { Incident, Schedule } from '../../types';
...
export const CollectorView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchSchedules = async () => {
      ...
      if (response.success && response.data?.results) {
        setWeeklySchedules(response.data.results);
        console.log('Weekly schedules fetched:', response.data.results);
      } else {
        setWeeklySchedules([]);
      }
  ...
  const handleStartRoute = (scheduleId: string) => {
    // TODO: Implémenter la logique de démarrage de route
    // Cela devrait appeler l'API pour mettre à jour le statut
    alert(`Démarrage de la route ${scheduleId}`);
  };
  ...
  const handleTruckStatusChange = async (newStatus: string) => {
    ...
    try {
      ...
      const response = await trucksAPI.updateStatus('1', newStatus); // ID du camion à adapter
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="p-6">
          ...
          {activeTab === 'schedule' && (
            <div>
              <h2 className="text-lg font-semibold text-gray-900 mb-4">
                Planning du jour - Équipe {userTeamId}
              </h2>
          ...
          {activeTab === 'reports' && (
            <div>
              <h2 className="text-lg font-semibold text-gray-900 mb-4">Signalements à traiter</h2>
              {myReports.length > 0 ? (
                <div className="space-y-4">
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="font-medium text-gray-900">
                          {report.type === 'overflow' ? 'Débordement' :
                           ...
                           report.type === 'missed_collection' ? 'Collecte manquée' : 'Autre'}
                        </h3>
                        <div className="flex items-center space-x-2">
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                            ...
                            {report.priority === 'high' ? 'Haute' :
                             report.priority === 'medium' ? 'Moyenne' : 'Faible'} priorité
                          </span>
                        </div>
                      </div>
